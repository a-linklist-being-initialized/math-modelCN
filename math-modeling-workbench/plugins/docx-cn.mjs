// docx-cn Skill — 标准宿主插件（agent preset 持久化版）
//
// 零依赖 ESM 模块：把随预设持久化的 skills/docx-cn/SKILL.md 注册为
// 可被 skill 工具加载的内置 skill（与 math-modeling 插件同一机制）。
// 只消费宿主 skills 服务，不发布任何服务，无需 isolate realm。
// 路径从插件自身位置推导（插件位于 <预设>/plugins/，skill 位于 <预设>/skills/docx-cn），
...
try {
  if (__here) {
    const sk = new URL('../skills/docx-cn', __here)
    let p = sk.pathname
...
export async function apply(ctx) {
  ...
  try {
    ...
    if (info) {
      ...
      console.error('[docx-cn] 内置 skill docx-cn 已注册')
    } else {
      console.error('[docx-cn] SKILL.md 不存在: ' + mainMd)
    }
  } catch (e) {
    console.error('[docx-cn] 内置 skill 注册失败: ' + String((e && e.message) || e))
  }
}
